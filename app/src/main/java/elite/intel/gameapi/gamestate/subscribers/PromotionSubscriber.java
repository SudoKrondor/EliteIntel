...
public class PromotionSubscriber {
    ...
    public void onPromotionEvent(PromotionEvent event) {
        ...
        if (imperialNavyRank != null) {
            ...
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.imperialNavy", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        ...
        if (federalNavyRank != null) {
            ...
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.federalNavy", string), "Congratulate the commander on their promotion and state the new rank clearly.");
            rankAndProgressDto.setCombatRankFederation(event.getFederation());
            session.setRankAndProgressDto(rankAndProgressDto);
        }
        if (combatRank != null) {
            String string = Ranks.getCombatRankMap().get(combatRank);
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.combat", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        ...
        if (tradeRank != null) {
            ...
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.trade", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        }
        if (exploreRank != null) {
            String string = Ranks.getExplorationRankMap().get(exploreRank);
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.exploration", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        }
        if (exobiologyRank != null) {
            String string = Ranks.getExobiologyRankMap().get(exobiologyRank);
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.exobiology", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        }
        if (soldier != null) {
            String string = Ranks.getMercenaryRankMap().get(soldier);
            CompanionRuntime.narrator().narrate(localizedEvent("event.promotion.soldier", string), "Congratulate the commander on their promotion and state the new rank clearly.");
        }
    }
}
