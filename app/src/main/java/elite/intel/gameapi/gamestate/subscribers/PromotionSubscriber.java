...
public class PromotionSubscriber {
    ...
    public void onPromotionEvent(PromotionEvent event) {
        ...
        if (combatRank != null) {
            ...
            GameEventBus.publish(new SensorDataEvent(localizedEvent("event.promotion.combat", string), "Congratulate the commander on their promotion and state the new rank clearly."));
        ...
        if (tradeRank != null) {
            ...
            GameEventBus.publish(new SensorDataEvent(localizedEvent("event.promotion.trade", string), "Congratulate the commander on their promotion and state the new rank clearly."));
        }
        if (exploreRank != null) {
            String string = Ranks.getExplorationRankMap().get(exploreRank);
            GameEventBus.publish(new SensorDataEvent(localizedEvent("event.promotion.exploration", string), "Congratulate the commander on their promotion and state the new rank clearly."));
        }
        if (exobiologyRank != null) {
            String string = Ranks.getExobiologyRankMap().get(exobiologyRank);
            GameEventBus.publish(new SensorDataEvent(localizedEvent("event.promotion.exobiology", string), "Congratulate the commander on their promotion and state the new rank clearly."));
        }
        if (soldier != null) {
            String string = Ranks.getMercenaryRankMap().get(soldier);
            GameEventBus.publish(new SensorDataEvent(localizedEvent("event.promotion.soldier", string), "Congratulate the commander on their promotion and state the new rank clearly."));
        }
    }
}
