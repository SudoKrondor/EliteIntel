...
/**
 * Single owner of VEGA's runtime diagnostics: it renders each consciousness stage into one compact,
 * scannable SYSTEM LOG line and publishes it on the program-wide diagnostics bus ({@link UiBus} +
 * {@link AppLogEvent}/{@link AppLogDebugEvent}) - the same channel the legacy brain logs to, so VEGA and
 * legacy diagnostics share one surface. Call sites only choose the stage and its values; the line format, the
 * {@code VEGA} prefix, the per-thought trace tag and the info/detail routing live here (no call site
 ...
 * {@link VegaConfig#diagnosticsVerbose()} is on (the default while the detailed-log GUI toggle is pending),
 * they are promoted onto the always-visible channel.
 */
public final class VegaDiagnostics {

    /**
     * Longest inlined free text (input, spoken line, injected fact) before it is elided. The SYSTEM
     * LOG panel word-wraps, so this only bounds runaway text; it is generous enough that a normal VEGA sentence
    ...
    /**
     * Trace tag for a cross-thread stage that no single thought owns (barge-in, off-lane record compression).
     */
     ...
     * {@link VegaConfig#diagnosticsVerbose()} is on, which promotes it onto the always-visible channel.
     */
    ...
    /**
     * {@code VEGA <tag> <stage>: <detail>} - the one line format every VEGA diagnostic shares.
     */
    private static String format(String tag, String stage, String detail) {
        String head = "VEGA " + tag + " " + stage;
