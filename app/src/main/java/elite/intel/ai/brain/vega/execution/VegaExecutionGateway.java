 ...
 * does not block unrelated queries or the next commander's cognitive turn. Small VEGA system functions
 ...
 */
public final class VegaExecutionGateway implements ExecutionGateway {

    private static final Logger log = LogManager.getLogger(VegaExecutionGateway.class);
    ...
    public VegaExecutionGateway() {
        this(CommandHandlerFactory.getInstance().registerCommandHandlers(),
                ...
                Executors.newThreadPerTaskExecutor(virtualDaemon("vega-action")),
                Executors.newFixedThreadPool(VegaConfig.maxParallelQueryExecutions(),
                        daemon("vega-query")));
    }
     ...
     */
    VegaExecutionGateway(Map<String, IntelAction> commandHandlers,
    ...
    public CompletableFuture<JsonObject> submit(ExecutionRequest request) {
        if (closed.get()) {
            return CompletableFuture.failedFuture(
                    new RejectedExecutionException("VEGA execution gateway is closed"));
        }
        ...
        return CompletableFuture.failedFuture(
                new IllegalArgumentException("Unknown VEGA tool: " + toolName));
    }
        ...
        try {
            lane.execute(() -> {
                ...
                try {
                    ...
                    log.error("VEGA tool '{}' failed", request.toolName(), failure);
