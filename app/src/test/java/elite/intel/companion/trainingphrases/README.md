...
| Файл | Назначение |
|---|---|
| `TrainingPhraseQualityProbe.java` | Скоринг: гоняет пробы через эмбеддер, пишет `build/training-phrase-quality-ru.csv` |
| `SemanticReducerProbe.java` | Диагностика: `dumpsCatalog` (каталог), `dumpsParams` (схема параметров), `dumpsAntonymCosines` (косинусы пар) |
| `../../../resources/trainingphrases/probe-phrases-ru.json` | Пробы: `{ "id": [10 фраз], ... }` — тестовые формулировки на каждую команду |
...
# 2) прогнать скоринг -> build/training-phrase-quality-ru.csv
