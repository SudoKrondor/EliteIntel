...
class FleetCarrierRouteManagerTest {
    ...
    // -- the route is only ever written by the commander's own plot ----------------

    /**
     * The table used to hold legs the truncating read hides, and the app used to ask that read whether
     * there was a route at all. A carrier parked on the last leg of a route therefore looked route-less,
     * its rows survived the next arrival, and the one it had already flown came back as leg 1.
     */
    @Test
    void aRouteTheCarrierIsSittingAtTheEndOfIsStillAStoredRoute() {
        session.setLastKnownCarrierLocation("Sol");
        route.setFleetCarrierRoute(plot("Maia"));
        session.setLastKnownCarrierLocation("Maia");

        assertTrue(route.getFleetCarrierRoute().isEmpty(), "nothing left to fly");
        assertTrue(route.hasStoredLegs(), "but the leg is still on file, and an arrival must be able to see it");
    }

    @Test
    void aClearedRouteHasNoStoredLegs() {
        session.setLastKnownCarrierLocation("Sol");
        route.setFleetCarrierRoute(plot("Maia", "Merope"));
        ...
        assertTrue(route.getFleetCarrierRoute().isEmpty());
    }
