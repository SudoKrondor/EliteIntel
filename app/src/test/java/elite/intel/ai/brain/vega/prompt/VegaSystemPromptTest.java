...
class VegaSystemPromptTest {
    ...
    /**
     * Both spoken sources must open by saying what VEGA is: an AI, named, and not hedging about it.
     *
     ...
     * {@code VegaIdentity.identityClause()}, now opens every prompt whose output is spoken.
     */
    @Test
    void bothSpokenPromptsOpenWithTheSameAiIdentity() {
        String identity = VegaIdentity.identityClause();
        assertTrue(identity.contains("artificial intelligence"), "VEGA must be named as an AI, not merely 'the AI'");
        assertTrue(identity.contains(VegaIdentity.name()));
    ...
    /**
     * The product invariant: VEGA's job is to emit an action, and conversation is only what happens when
     ...
     * USER/ASSISTANT pairs, so whatever VEGA said last turn is in context as an example of what to
    ...
    /**
     * How VEGA speaks of itself follows the voice the active ship carries, in both spoken prompts.
     *
