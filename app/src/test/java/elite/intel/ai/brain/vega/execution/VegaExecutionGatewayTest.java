 ...
 */
class VegaExecutionGatewayTest {
    ...
    void unknownToolFailsTheFuture() {
        VegaExecutionGateway gateway = new VegaExecutionGateway(Map.of(), Map.of(), Map.of(), SYNC, SYNC);
    ...
    void oldRequestGenerationCannotRouteStaticSpeechIntoTheCurrentRuntime() throws Exception {
        VegaRuntimeGraph oldRuntime = VegaRuntimeTestSupport.installNarrator(VegaNarrator.NO_OP);
        long oldGenerationId = oldRuntime.runtimeGeneration().generationId();
        VegaRuntimeTestSupport.uninstall(oldRuntime);
        AtomicInteger currentNarratorCalls = new AtomicInteger();
        VegaNarrator currentNarrator = new VegaNarrator() {
            @Override
        ...
        VegaRuntimeGraph currentRuntime = VegaRuntimeTestSupport.installNarrator(currentNarrator);
        IntelAction oldHandler = new IntelAction() {
            ...
            @Override
            public JsonObject handle(String action, JsonObject params, String text) {
                VegaRuntime.narrator().filler("late result", false);
                return null;
            }
        };
        VegaExecutionGateway gateway = new VegaExecutionGateway(
        ...
        try {
            ...
            gateway.close();
            VegaRuntimeTestSupport.uninstall(currentRuntime);
        }
    }
}
