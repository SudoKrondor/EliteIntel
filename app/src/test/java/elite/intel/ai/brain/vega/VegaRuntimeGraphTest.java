...
class VegaRuntimeGraphTest {
    ...
    void closeFencesGenerationCancelsOwnedWorkAndClosesDependenciesOnce() {
        ...
        VegaRuntimeGraph runtimeGraph = graph(
                runtimeGeneration, llmGateway, speechGateway, executionGateway, VegaNarrator.NO_OP);
    ...
    void generationIdentifiersIncreaseMonotonically() {
        VegaRuntimeGeneration first = new VegaRuntimeGeneration();
        VegaRuntimeGeneration second = new VegaRuntimeGeneration();

        assertTrue(second.generationId() > first.generationId());
    }

    private static VegaRuntimeGraph graph(
