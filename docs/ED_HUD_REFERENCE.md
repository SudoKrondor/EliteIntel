...
- **Overlay:** `HUD_FONT_OVERLAY` — отдельная роль компактного диалога компаньона над игрой;
  сейчас равна ступени MD, но не разделяет её семантику.
...
Для always-on-top overlay используется отдельный профиль `HudLogArea.overlayChat(delay)`: он
сохраняет две дорожки и анимацию, но берёт `HUD_FONT_OVERLAY`, не рисует временные метки и строку
ожидания CMDR с курсором. Это не меняет стандартный `HudLogArea.chat`. Контроллер overlay сам
очищает предыдущую пару при новой реплике CMDR, поэтому на экране остаётся только текущий обмен.
Карточки разговора и маршрута скрывают шапку `HudSection`; их общий фон —
`HUD_COLOR_ROLE_OVERLAY_CARD_BACKGROUND` с прозрачностью 30 %, а canvas и viewport разговора
прозрачны, чтобы не усиливать затемнение. Торговый маршрут — не таблица: его верхняя строка несёт
две пары через стрелку — действие → груз и прибыль → сумма с `CR`; вторая показывает систему →
станцию и текущую точку, третья — следующий прыжок и число прыжков только при активной навигации.
Нативная оболочка берёт
`HUD_COLOR_ROLE_OVERLAY_TRANSPARENT_BACKGROUND`, когда рабочий стол поддерживает попиксельную
прозрачность; без композитора она штатно переходит на непрозрачный
`HUD_COLOR_ROLE_APPLICATION_BACKGROUND`, без платформенных API. Карточка маршрута располагается
над разговором; обычная навигация в ней собирается через `HudTwoColumns` (§9), а разговор использует компактную высоту
`HUD_OVERLAY_CONVERSATION_HEIGHT`; ширина оболочки задаётся компактным
`HUD_OVERLAY_DEFAULT_WIDTH`.
