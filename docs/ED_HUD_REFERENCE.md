...
реплика Vega получает непрозрачную рейку, угловые скобы и циановую заливку, затухающую от рейки к
тексту. Это осознанное исключение из запрета градиентов §0. После последнего символа печати
заливка, рейка и скобы плавно затухают до обычного вида за `HUD_CHAT_ACTIVE_HOLD_MS`.
