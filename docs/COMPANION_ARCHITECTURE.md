...
> - **Исполнение не является диалогом:** `COMMAND`, `MACRO`, reflex-command, immediate ack, handler outcome/failure, dangerous confirmation и `request_input` не пишут conversational memory. Их речь и runtime-поведение не меняются. `QUERY` сохраняет отдельный структурированный контракт: input + при необходимости `<processing/>` + связанная пара CALL/RESULT.
...
Каждый ход командира LLM вызывает `classify_turn` ровно один раз: он несёт `topic`, `importance`, `is_question` и `canonical_fact`. Тема — глобальная и липкая, меняется **только** через `classify_turn`; важность — пер-ход. Host оставляет fact text только у не-вопроса с `importance=HIGH`, если этот ход не вызвал game action и модель прислала непустой canonical. Canonical обязан иметь exact meaningful-token overlap с текущей репликой; при ungrounded copy host использует исходную HIGH-реплику. `NORMAL`/`LOW`, вопросы и game-action turns получают пустой canonical независимо от ответа модели. `MAX` хранит исходную реплику дословно.
...
* `QUERY` → один раз записать `[COMMANDER]`, при detached-выполнении закрыть его `<processing/>`, затем добавить связанную CALL/RESULT-пару;
...
Смешанный `QUERY+COMMAND` получает ровно один input-якорь от QUERY; командная часть остаётся memory-silent. Пара чистого разговора выглядит так:
...
```text
[COMMANDER] question
[COMPANION] <processing/>        # только если query ещё выполняется
[COMPANION/CALL] query call      # CALL и RESULT публикуются по готовности результата
...
При interrupt `CommanderThought` не начинает новых действий и не пытается «дозаписать» вход. Если settlement ещё не создал разговорную пару или QUERY-якорь, `currentInput` просто исчезает вместе с `ThoughtContext`; причина остаётся в diagnostics.
...
│  └─ facts             MemoryFactSource, LocalizedFactRelevance, MemoryFactGatherer,
│                       MergedFactCandidates, current-state fact sources
