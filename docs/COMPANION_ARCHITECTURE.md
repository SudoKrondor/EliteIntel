...
* фильтр — word-overlap, толерантный к словоформам (`CompanionWordMatch`), а не contiguous-вхождение; пустой query → просто самые свежие записи;
* результаты всех источников объединяются и сортируются **по релевантности в первую очередь** (число совпавших слов), затем по важности, затем по свежести — чтобы сильно релевантный рутинный факт обходил слабо совпавший MAX, а накопленный архив не монополизировал выдачу;
