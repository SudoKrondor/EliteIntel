...
* **Итог tool-call по типу действия (`CommanderThought.recordOutcome`, v0.15; скорректировано после отката `CommandOutcome`).** Тип резолвит `IntelActionTypeResolver` (`companion.tools`, инжектируемый тест-сим) → `COMMAND/QUERY/MACRO/SYSTEM/UNKNOWN`. Озвучка/память: COMMAND — самохозяин речи (handler снова self-narrating через старый voice path); companion только пишет `[TOOL_RESULT]` «command id executed»+текст/описание и не добавляет fallback-ack для пустого результата. QUERY — `text_to_speech_response` озвучивается и пишется в память как `[COMPANION]`; MACRO — молча, память «macro id executed»+описание; SYSTEM/UNKNOWN — речь и timeline не трогаем. Описание берётся из снапшота tools (`LlmToolDefinition.description()`), не из реестра. Команды/запросы синхронны (fire-and-forget откатан). `silentInCompanion()` удалён.
