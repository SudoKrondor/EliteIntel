...
missing `app/src/main/java/elite/intel/ui/view/HudDialogHeader.java` (135 lines) — the class that was reported as missing in `SYNC_REPORT.md` (causing
...
- **PTT/Input tab intact** — `InputSettingsPanel.java` and `SdlInputService.java` both
  present and tracked.
- **`..` intact** — all 17 tracked files present:
...
- **Pre-existing uncommitted change**: `SYNC_REPORT.md` has an
