...
SDL3 joystick and gamepad input via `SDL_INIT_JOYSTICK | SDL_INIT_GAMEPAD` on Windows uses XInput and DirectInput — Microsoft-signed, well-known APIs. The SDL3 native library `SDL3.dll` was verified clean on VirusTotal with a score of 0/63 on June 9 2026. See `..` for the verification hash.
