...
1. **Button-type** — has a Primary and/or Secondary input slot; either slot may be unbound; may carry an element-wide toggle flag covering both slots as one shared behaviour (not per-slot).
...
**A standalone setting's value has three shapes** (measured across the specimen files, 2026-09-16; 93 of the 515 elements in `Custom.4.2.binds` are this kind): **boolean**, always `0` or `1` (36); **float**, written to eight decimal places (25); and **enum**, a token naming one choice from a fixed set — `Bindings_MouseYaw`, `mute_toggle` — of which 23 of 31 sit empty, meaning the game's default rather than an absent value (31). `KeyboardLayout` alone carries no `Value` attribute, holding its value as element text. **Real files contain values no vocabulary knows** (measured 2026-09-17 across 48,354 community files): hand-edited tokens like `Bindings_ThrottleRangeFullRange` and `Bindings_MouseRole`, binding tokens pasted into settings fields, eight files carrying the game's *other* settings (`Language`, market filters, route plotting) under a `.binds` extension, and files with `Deadzone`, `Inverted`, `ToggleOn` or `Hold` as top-level elements. **The model must round-trip what it does not understand** — preserve the value, never correct it to the nearest legal one, never drop an unknown element — because the game has been reading these files without complaint. **The `MajorVersion`/`MinorVersion` attributes are optional and untrustworthy:** 1,225 of those files carry no version at all (1,078 using an older `<Root PresetName=… SortOrder="0">` schema, 147 a bare `<Root>`), and every reading above the game's own 4.2 turned out to be a third-party writer's stamp — 116 files reading `4.5` are the GameGlass app, `6.1` is Virpil, `4.4` is a binds editor. Nothing may gate behaviour on the version. **Float formatting varies between Frontier's own files** — `HeadlookIncrement` is `0.00000000` in most shipped presets and `0.00` in others, so a parser must not assume eight decimal places, and a writer should preserve what it read (observed 2026-09-16 across the 90 preset files Elite ships). **The element set varies too:** `GunsightSystem` appears in Frontier's presets and in none of the commander files collected here, so a file legitimately lacks elements the game supports, and BindForge must neither invent them nor treat their absence as damage. **An enum's legal vocabulary cannot be read from a file** — a file holds one token, not the set — which is why [BindForge captures them by testing](../02-features/bindforge/bind-editor.md#settings-entries-are-rows-too--settled-2026-09-16) before offering a dropdown.

**Design principle:** the three kinds are modelled with distinct shapes, not one flat structure with a lot of "only applies sometimes" fields. This prevents assigning an axis-only property to a button element (or vice versa) as a category of error at the modelling level itself.
...
**Critical behavioural caveat:** the raw parsed values for a slot's main key versus its modifiers must be preserved exactly as read, for accurate round-tripping — but must never be trusted at face value by anything that actually presses keys to fire a binding. The game's own capture process does not treat "main key" versus "modifier" as meaningful category labels; it only tracks press order, so whichever key was released last ends up recorded as the slot's main value, even if that key is conceptually the modifier. Anything executing a binding must classify each key by its own identity (is this literally one of the recognised modifier keys, regardless of which slot position it landed in) rather than trusting slot position. See [Binding Conflict Rules](../02-features/bindforge/domain-knowledge/EliteDangerous-ConflictRules.md) for the confirmed behaviour this derives from.
...
## Open Question — Modelling Philosophy
